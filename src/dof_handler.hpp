...
public:
  ...
  struct CellAccessor
  {
    ...
    ::FaceAccessor<dim, 1> face(LocalIndexType local_f) const
    {
    ...
    LocalIndexType neighbor_face_index(LocalIndexType local_f) const
    {
      ...
      for (LocalIndexType lf = 0; lf < ::SimplexTopology<dim, 1>::faces_per_cell;
           ++lf)
  ...
  struct ActiveCellRange
  {
    ...
    struct Iterator
    {
      typename Triangulation<dim, 1>::ActiveCellRange::Iterator tria_it;
      const DoFHandler* handler;
  ...
  DoFHandler(const Triangulation<dim, 1>& tria,
  ...
  const Triangulation<dim, 1>& tria() const { return tria_; }
...
private:
  const Triangulation<dim, 1>& tria_;
