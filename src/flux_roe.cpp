...
flux_roe(Tensor<1,4,double> UL, Tensor<1,4,double> UR, Tensor<1,2,double> n) {
    ...
    double smag = std::max(std::abs(uL*nx + vL*ny) + std::sqrt(gamma*pL/rhoL),
    ...
    for(int i=0; i<4; ++i) {
        if (std::abs(lambda[i]) < eps) lambda[i] = 0.5*(lambda[i]*lambda[i]/eps + eps);
        else lambda[i] = std::abs(lambda[i]);
    }

    // 4. Dissipation (Simplified 2x2 for brevity, use full 4-wave logic in production)
