...
public:
                ...
                const Tensor<1, dim, RealType>& normal)
  {
    ...
    double p0 =
      1 / gamma; // rho0*a0^2/gamma but rho0 and a0 are 1 b/c nondimensional
    double pout = 0.7 * p0;
    double Mout2 =
      2.0 / (gamma - 1.0) * (std::pow(p0 / pout, (gamma - 1.0) / gamma) - 1);
    double qout = 0.5 * gamma * pout * Mout2;
    const auto q2 = (momentum_x * momentum_x + momentum_y * momentum_y) / rho;
    const auto p =
      (Parameters<RealType>::gamma - 1.0) * (energy - 0.5 * rho * q2);

    // Calculate pressure coefficient
    const auto cp =
      (p - Parameters<RealType>::pout) / Parameters<RealType>::qout;
  ...
  std::vector<double> calcForceCoeffs(
